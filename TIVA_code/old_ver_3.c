...
static portTASK_FUNCTION(productora1,pvParameters)
{
    INFO_COLA info_cola;
    info_cola.IDProd=1;
    ...
    while (1)
    {
        ...
        vTaskDelay(((rand() % 3) + 1) * configTICK_RATE_HZ); // Tiempo que tarda en producir la tarea tiempo random
...
//C�digo productora 2
static portTASK_FUNCTION(productora2,pvParameters)
{
    INFO_COLA info_cola;
        info_cola.IDProd=2;
        ...
        while (1)
        {
            ...
            vTaskDelay(4 * configTICK_RATE_HZ); // Tiempo que tarda en producir la tarea tiempo random 4 segundos
            MAP_GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, GPIO_PIN_3); //luz verde
            xQueueSend(cola_prod_cons_1, &info_cola, portMAX_DELAY);
            MAP_GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, 0);          //luz verde
            counter++;
    }
}
...
static portTASK_FUNCTION(consumidora1,pvParameters)
{
    ...
    while (1)
    {
        xQueueReceive(cola_prod_cons_1, &info_cola, portMAX_DELAY);
...
int main(void)
{
    ...
    //Habilitamos puerto
    // Habilita el perif�rico: Puerto F (LEDs)
    ...
    if ((xTaskCreate(productora1, "Productor1", 128, NULL, tskIDLE_PRIORITY + 1,
    NULL) != pdPASS))
    {
        while (1)
        {
    ...
    if ((xTaskCreate(productora2, "Productor2", 128, NULL, tskIDLE_PRIORITY + 1,
        NULL) != pdPASS))
        {
            while (1)
            {
