...
static portTASK_FUNCTION(productora1,pvParameters)
{
    ...
    {
        int32_t N = 10000;
        int32_t kid_id = (rand() % N) + 1;
        vTaskDelay(3 * configTICK_RATE_HZ); // Tiempo que tarda en producir la tarea
        xQueueSend(cola_prod_cons_1, &kid_id, portMAX_DELAY);
        counter++;
    }
...
static portTASK_FUNCTION(consumidora1,pvParameters)
{
    ...
    while (1)
    {
        xQueueReceive(cola_prod_cons_1, &kid_id ,portMAX_DELAY);
        vTaskDelay(3 * configTICK_RATE_HZ);
        PARAM_MENSAJE_CONTADOR parametro;
        parametro.numMensajes = counter;
        parametro.id = kid_id; //lee el kid ID de la tarea productora
        //El mensaje esta bien pero no esta implementado
...
int main(void)
{
    ...
    if ((xTaskCreate(productora1, "Productor", 128, NULL, tskIDLE_PRIORITY + 1,
                     NULL) != pdPASS))
    {
        while (1)
        {
    ...
    cola_prod_cons_1 = xQueueCreate(3, sizeof(uint32_t));
